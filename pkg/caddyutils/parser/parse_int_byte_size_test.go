...
var _ = Describe("ParseByteSize", func() {
	Context("ParseByteSize", func() {
		...
		testcase := func(input string, expected int, opts ...parser.Option) Assertion {
			...
			err = parser.ParseIntByteSize(dispenser, &dest, opts...)
			if err != nil {
		...
		Context("With Default() option", func() {
			...
			It("should ignore default option when value is valid", func() {
				testcase("test 2KiB", 2048, parser.Default(1)).Should(Succeed())
				testcase("test 2KB", 2000, parser.Default(1)).Should(Succeed())
				testcase("test 2K", 2000, parser.Default(1)).Should(Succeed())
			...
			It("should ignore default option when value is invalid", func() {
				...
				testcase("test 3.4.0", 0, parser.Default(1)).Should(Fail())
			})
		})
		...
		Context("With Inplace() option", func() {
			...
			Context("With Default() option", func() {
				...
				It("should ignore default option when value is valid", func() {
					testcase("2MiB", 2097152, parser.Inplace(), parser.Default(1)).Should(Succeed())
				})
			})
		})
	})
