...
var _ = Describe("ParseInt32ByteSize", func() {
	Context("ParseInt32ByteSize", func() {
		var Fail = HaveOccurred
		testcase := func(input string, expected int32, opts ...parser.Option) Assertion {
			var err error
			dispenser := NewTestDispenser(input)
			var dest int32
			err = parser.ExpectString(dispenser)
			Expect(err).NotTo(HaveOccurred())
			err = parser.ParseInt32ByteSize(dispenser, &dest, opts...)
			if err != nil {
		...
		Context("With Default() option", func() {
			It("should use default option when empty", func() {
				testcase("test", 1, parser.Default(int32(1))).Should(Succeed())
			})
			It("should ignore default option when value is valid", func() {
				testcase("test 2KiB", 2048, parser.Default(int32(1))).Should(Succeed())
				testcase("test 2KB", 2000, parser.Default(int32(1))).Should(Succeed())
				testcase("test 2K", 2000, parser.Default(int32(1))).Should(Succeed())
				testcase("test 2", 2, parser.Default(int32(1))).Should(Succeed())
			})
			It("should ignore default option when value is invalid empty string", func() {
				testcase(`test ""`, 0, parser.Default(int32(1))).Should(Fail())
			})
			It("should ignore default option when value is invalid", func() {
				testcase("test invalid", 0, parser.Default(int32(1))).Should(Fail())
				testcase("test 3.4.0", 0, parser.Default(int32(1))).Should(Fail())
			})
		})
		...
		Context("With Inplace() option", func() {
			...
			Context("With Default() option", func() {
				It("should use default option when empty", func() {
					testcase(`""`, 1, parser.Inplace(), parser.Default(int32(1))).Should(Succeed())
				})
				It("should ignore default option when value is valid", func() {
					testcase("2MiB", 2097152, parser.Inplace(), parser.Default(int32(1))).Should(Succeed())
				})
			})
		})
	})
