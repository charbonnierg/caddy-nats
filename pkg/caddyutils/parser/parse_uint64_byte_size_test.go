...
var _ = Describe("ParseUint64ByteSize", func() {
	Context("ParseUint64ByteSize", func() {
		var Fail = HaveOccurred
		testcase := func(input string, expected uint64, opts ...parser.Option) Assertion {
			var err error
			dispenser := NewTestDispenser(input)
			var dest uint64
			err = parser.ExpectString(dispenser)
			Expect(err).NotTo(HaveOccurred())
			err = parser.ParseUint64ByteSize(dispenser, &dest, opts...)
			if err != nil {
		...
		Context("With Default() option", func() {
			It("should use default option when empty", func() {
				testcase("test", 1, parser.Default(uint64(1))).Should(Succeed())
			})
			It("should ignore default option when value is valid", func() {
				testcase("test 2KiB", 2048, parser.Default(uint64(1))).Should(Succeed())
				testcase("test 2KB", 2000, parser.Default(uint64(1))).Should(Succeed())
				testcase("test 2K", 2000, parser.Default(uint64(1))).Should(Succeed())
				testcase("test 2", 2, parser.Default(uint64(1))).Should(Succeed())
			})
			It("should ignore default option when value is invalid empty string", func() {
				testcase(`test ""`, 0, parser.Default(uint64(1))).Should(Fail())
			})
			It("should ignore default option when value is invalid", func() {
				testcase("test invalid", 0, parser.Default(uint64(1))).Should(Fail())
				testcase("test 3.4.0", 0, parser.Default(uint64(1))).Should(Fail())
			})
		})
		...
		Context("With Inplace() option", func() {
			...
			Context("With Default() option", func() {
				It("should use default option when empty", func() {
					testcase(`""`, 1, parser.Inplace(), parser.Default(uint64(1))).Should(Succeed())
				})
				It("should ignore default option when value is valid", func() {
					testcase("2MiB", 2097152, parser.Inplace(), parser.Default(uint64(1))).Should(Succeed())
				})
			})
		})
	})
