...
var _ = Describe("ParseUint32", func() {
	Context("ParseUint32", func() {
		var Fail = HaveOccurred
		testcase := func(input string, expected uint32, opts ...parser.Option) Assertion {
			var err error
			dispenser := NewTestDispenser(input)
			var dest uint32
			err = parser.ExpectString(dispenser)
			Expect(err).NotTo(HaveOccurred())
			err = parser.ParseUint32(dispenser, &dest, opts...)
			if err != nil {
		...
		Context("With Default() option", func() {
			It("should use default option when empty", func() {
				testcase("test", 1, parser.Default(uint32(1))).Should(Succeed())
			})
			It("should ignore default option when value is valid", func() {
				testcase("test 2", 2, parser.Default(uint32(1))).Should(Succeed())
			})
			It("should ignore default option when value is invalid empty string", func() {
				testcase(`test ""`, 0, parser.Default(uint32(1))).Should(Fail())
			})
			It("should ignore default option when value is invalid", func() {
				testcase("test invalid", 0, parser.Default(uint32(1))).Should(Fail())
				testcase("test 1.0", 0, parser.Default(uint32(1))).Should(Fail())
				testcase("test -1", 0).Should(Fail())
			})
		})
		...
		Context("With Inplace() option", func() {
			...
			Context("With Default() option", func() {
				It("should use default option when empty", func() {
					testcase(`""`, 1, parser.Inplace(), parser.Default(uint32(1))).Should(Succeed())
				})
				It("should ignore default option when value is valid", func() {
					testcase("2", 2, parser.Inplace(), parser.Default(uint32(1))).Should(Succeed())
				})
			})
		})
	})
